...
const state = {
  exercises: clone(FALLBACK_EXERCISES),
...
function openSimpleModal(type) {
  const entries = {
    'edit-profile': ['user-round', 'Dados da conta', `${escapeHTML(state.user.nome)}<br>${escapeHTML(state.user.email)}`],
...
async function initializeApp() {
  try {
    ...
    state.exercises = [...clone(FALLBACK_EXERCISES), ...state.customExercises];
