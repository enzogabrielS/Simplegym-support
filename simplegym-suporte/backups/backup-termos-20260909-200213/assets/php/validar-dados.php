...
function validarDados(array $dados): array
{
    ...
    foreach ($customizados as $exercicio) {
        ...
        validarCondicao(is_array($exercicio['muscleGroups'] ?? null) && array_is_list($exercicio['muscleGroups']) && count($exercicio['muscleGroups']) > 0);
    ...
    foreach ($resultado['schedule'] as $dia => $vazio) {
        ...
        validarCondicao(is_array($programados) && array_is_list($programados) && count($programados) <= 100);
