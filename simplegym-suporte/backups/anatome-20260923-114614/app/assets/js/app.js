...
// Cargas são sempre armazenadas em kg. A preferência altera apenas a apresentação.
function displayedWeight(kg) {
  return Math.round(Number(kg) * (state.weightUnit === 'lb' ? 1 / 0.45359237 : 1) * 100) / 100;
}
function weightText(config) { return config.weight > 0 ? `${displayedWeight(config.weight)} ${state.weightUnit}` : 'peso corporal'; }
...
function configurationControls(config, action = 'change-config', attributes = '') {
  ...
  if (!isCardioExercise(config.exerciseId)) fields.push(['weight', 'Carga', displayedWeight(config.weight), ` ${state.weightUnit}`]);
...
function adjustConfig(config, field, delta) {
  ...
  const increment = Number(delta) * (field === 'weight' && state.weightUnit === 'lb' ? 0.45359237 : 1);
  const precision = field === 'weight' ? 100000000 : 100;
  config[field] = Math.min(max, Math.max(min, Math.round((Number(config[field]) + increment) * precision) / precision));
  const value = modalContent.querySelector(`[data-config-value="${field}"]`);
  if (value) value.textContent = (field === 'weight' ? displayedWeight(config[field]) : config[field]) + value.dataset.suffix;
}
...
function openWeightUnit() {
  openModal(`<div class="simple-icon"><i data-lucide="scale"></i></div><h2 class="modal-title" id="modal-title">Unidade de carga</h2><p class="simple-copy">Escolha como deseja ver e ajustar suas cargas. Os valores são convertidos automaticamente, sem alterar o peso dos treinos salvos.</p><div class="theme-options">${[['kg', 'Quilogramas', 'Padrão brasileiro'], ['lb', 'Libras', '1 lb = 0,45359237 kg']].map(([unit, label, detail]) => `<button class="theme-option ${state.weightUnit === unit ? 'selected' : ''}" data-action="set-weight-unit" data-unit="${unit}" aria-pressed="${state.weightUnit === unit}"><strong>${unit}</strong><span><strong>${label}</strong><small>${detail}</small></span><i data-lucide="${state.weightUnit === unit ? 'check-circle-2' : 'circle'}"></i></button>`).join('')}</div><p class="simple-copy">Os botões +1, −1, +5 e −5 usam a unidade selecionada.</p><button class="modal-cta secondary" data-action="close-modal">Concluir</button>`);
}
async function setWeightUnit(unit) {
  if (!['kg', 'lb'].includes(unit) || unit === state.weightUnit) return;
