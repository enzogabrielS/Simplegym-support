...
class Client {
  ...
  api(file, data, csrf = true) { return this.request('/assets/php/' + file + '.php', file === 'cadastrar' && data ? { termsAccepted: '1.0', ...data } : data, csrf); }
}
...
try {
  ...
  check((await clientA.request('/assets/php/cadastrar.php', {name:'Teste',email:emails[0],password})).status, 422);
  check((await clientA.api('cadastrar', {name:'Teste',email:emails[0],password,termsAccepted:false})).status, 422);
  check((await clientA.api('cadastrar', {name:'Teste',email:emails[0],password,termsAccepted:'0.9'})).status, 422);
  ...
  assert.ok(original.catalog.exercises.some(e => e.id === 'supino-reto'));
  assert.ok(original.catalog.muscleGroups.some(g => g.id === 'peito'));
  assert.ok(original.catalog.levels.some(l => l.level === 1 && l.minXp === 0));
  assertions += 3;
  ...
  const inflated = structuredClone(state);
  inflated.totalWorkouts = 999;
  check((await clientA.api('dados', {state:inflated,version:1})).status, 200);
  check((await clientA.api('dados')).body.state.totalWorkouts, 1);
  check((await clientA.api('dados', { state, version: 0 })).status, 409);
  const invalid = structuredClone(state);
  invalid.plans[0].exercises[0].sets = -1;
  check((await clientA.api('dados', { state: invalid, version: 2 })).status, 422);
  ...
  const stateB = structuredClone(original.state);
  stateB.customExercises = [{ ...state.customExercises[0], name: 'Exercício exclusivo da conta B', description: 'Descrição pessoal B.' }];
  check((await clientB.api('dados', { state: stateB, version: 0 })).status, 200);
  check((await clientB.api('dados')).body.state.customExercises[0].name, 'Exercício exclusivo da conta B');
  check((await clientA.api('dados')).body.state.customExercises[0].name, 'Exercício pessoal');
  const foreignReference = structuredClone(original.state);
  foreignReference.plans = [{ ...state.plans[0], exercises: [{ exerciseId: 'personalizado-teste', sets: 3, reps: 12, weight: 0 }] }];
  check((await clientB.api('dados', { state: foreignReference, version: 1 })).status, 422);
  const helper = fileURLToPath(new URL('./verificar-tabelas.php', import.meta.url));
  execFileSync(php, ['-d', `extension_dir=${ext}`, '-d', 'extension=pdo_mysql', helper, ...emails], { stdio: 'inherit' });
  assertions++;
  const originalName = original.catalog.exercises.find(e => e.id === 'supino-reto').name;
  try {
    execFileSync(php, ['-d', `extension_dir=${ext}`, '-d', 'extension=pdo_mysql', helper, '--nome-catalogo', 'Supino lido diretamente do banco']);
    check((await clientA.api('dados')).body.catalog.exercises.find(e => e.id === 'supino-reto').name, 'Supino lido diretamente do banco');
  } finally {
    execFileSync(php, ['-d', `extension_dir=${ext}`, '-d', 'extension=pdo_mysql', helper, '--nome-catalogo', originalName]);
  }
