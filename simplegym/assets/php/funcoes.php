...
function usuarioAtual(): ?array
{
    ...
    $_SESSION['credencial'] = hash('sha256', $usuario['senha_hash']);
    unset($usuario['token_hash'], $usuario['senha_hash']);
...
function iniciarLogin(int $id): void
{
    ...
    $conecta = conectarBanco();
    $credencial = $conecta->prepare('SELECT senha_hash FROM usuarios WHERE id = ?');
    $credencial->execute([$id]);
    $_SESSION['credencial'] = hash('sha256', (string) $credencial->fetchColumn());
